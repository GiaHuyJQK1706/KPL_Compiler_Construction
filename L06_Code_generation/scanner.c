...
void skipBlank() { // Hàm này bỏ qua các ký tự trắng
  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_SPACE))
    readChar();
}

void skipComment() { // Hàm này bỏ qua comment (* <comment> *)
  int state = 0;
...
Token* readIdentKeyword(void) { // Hàm này đọc một từ khóa hoặc một identifier
...
Token* readNumber(void) { // Hàm này đọc một số
...
Token* readConstChar(void) { // Hàm này đọc một ký tự
...
Token* getToken(void) { // Hàm này trả về token tiếp theo
  Token *token;
  int ln, cn;
...
Token* getValidToken(void) { // Hàm này trả về token hợp lệ
...
void printToken(Token *token) { // Hàm này in ra thông tin của token

  printf("%d-%d:", token->lineNo, token->colNo); // In ra dòng và cột của token
