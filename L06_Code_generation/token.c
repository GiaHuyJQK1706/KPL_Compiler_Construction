...
int keywordEq(char *kw, char *string) { // Hàm này kiểm tra xem một từ khóa có bằng một chuỗi không
  while ((*kw != '\0') && (*string != '\0')) {
    if (*kw != *string) break;
    kw ++; string ++;
  }
...
TokenType checkKeyword(char *string) { // Hàm này kiểm tra xem một chuỗi có phải là một từ khóa không
  int i;
...
Token* makeToken(TokenType tokenType, int lineNo, int colNo) { // Hàm này tạo một token mới
...
char *tokenToString(TokenType tokenType) { // Hàm này chuyển một token type thành chuỗi
  switch (tokenType) {
