...
CodeBlock* createCodeBlock(int maxSize) { // Hàm này tạo một code block mới
...
void freeCodeBlock(CodeBlock* codeBlock) { // Hàm này giải phóng một code block
  free(codeBlock->code);
  free(codeBlock);
}

int emitCode(CodeBlock* codeBlock, enum OpCode op, WORD p, WORD q) { // Hàm này sinh mã
...
// Các hàm sinh mã cho các instruction
...
void printInstruction(Instruction* inst) { // Hàm này in ra một instruction
  switch (inst->op) {
...
void printCodeBlock(CodeBlock* codeBlock) { // Hàm này in ra một code block
...
void loadCode(CodeBlock* codeBlock, FILE* f) { // Hàm này tải mã từ file
...
void saveCode(CodeBlock* codeBlock, FILE* f) { // Hàm này lưu mã vào file
